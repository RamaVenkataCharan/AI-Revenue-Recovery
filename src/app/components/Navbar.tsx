...
import { Shield, Play, Activity, ListChecks, FileText, Mic, CheckCircle2, RefreshCw, Sparkles, Home } from 'lucide-react';
...
export default function Navbar() {
  ...
  const navLinks = [
    { href: '/', label: 'Overview & Story', icon: Home },
    { href: '/dashboard', label: 'Executive Console', icon: Activity },
    { href: '/dashboard/prediction', label: 'AI Model Prediction', icon: Sparkles, badge: 'Live AI' },
          ...
          {navLinks.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className="rounded bg-emerald-950 px-1 py-0.2 text-[9px] font-mono text-emerald-400 border border-emerald-800 hidden md:inline">
                    {item.badge}
                  </span>
                )}
              </Link>
